package Model;

public class Data {
    public static Integer admin_id;
    public static  String admin_username;
    public static String path;
    public static String teacher_id;
    public static String teacher_name;
    public static Integer student_id;

    public static String[] gender = {"Male", "Female", "Others"};

    public static String[] status = {"Active", "Inactive", "Confirm"};

    public static String[] specialization = {"Math", "English", "Literature", "History", "Geography", "Physics", "Chemistry"};


    public static Integer temp_studentID;
    public static String temp_name;
    public static String temp_gender;
    public static Long temp_number;
    public static String temp_address;
    public static String temp_status;
    public static String temp_date;
    public static String temp_path;




    public static String temp_teacherID;
    public static String temp_teacherName;
    public static String temp_teacherEmail;
    public static String temp_teacherPassword;
    public static String temp_teacherSpecialized;
    public static String temp_teacherGender;
    public static String temp_teacherMobileNumber;
    public static String temp_teacherImagePath;
    public static String temp_teacherAddress;
    public static String temp_teacherStatus;


    public static String temp_appID;
    public static String temp_appName;
    public static String temp_appGender;
    public static String temp_appMobileNumber;
    public static String temp_appAddress;
    public static String temp_appDescription;
    public static String temp_appDiagnosis;
    public static String temp_appTreatment;
    public static String temp_appTeacher;
    public static String temp_appSpecialized;
    public static String temp_appStatus;

    public Data() {
    }
}